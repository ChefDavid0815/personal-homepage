...
function overview() {
  ...
  document.querySelector('meta[name="description"]').content = copy('ChefZC 的随笔：生活、NBA After Hours、Folio、AXIOM、Festival Toolkit 与 WIS TECH TANK 的制作手记。', 'Notes by ChefZC: life, NBA After Hours, Folio, AXIOM, Festival Toolkit and WIS TECH TANK.');
  root.innerHTML = `<section class="posts-heading" aria-labelledby="posts-title"><div class="posts-kicker"><span>[ ${copy('写在作品之外', 'THE NOTES BETWEEN THE WORK')} ]</span><span>CHEFZC / JOURNAL</span></div><div class="posts-heading-line"><h1 id="posts-title">POSTS<span>.</span></h1><span class="posts-mark" aria-hidden="true">↙</span></div><div class="posts-intro"><h2>${copy('一些想法，<br>不必急着变成作品。', 'Some thoughts.<br>Room to let them grow.')}</h2><p>${copy('关于做东西，也关于生活。<br>把值得留下的念头，写成一页。', 'On making things, and on living.<br>A page for the thoughts worth keeping.')}</p></div></section><section class="posts-collection" aria-label="${copy('所有文章', 'All posts')}"><div class="posts-section-label"><span>${copy('随笔与制作手记', 'NOTES & BUILD JOURNALS')} <b>${String(posts.length).padStart(2,'0')}</b></span><span>2026 / FIRST PAGES</span></div><div class="posts-grid">${posts.map((item, i) => card(item, i === 0)).join('')}</div></section><div class="posts-end"><span>TO BE CONTINUED</span><p>${copy('下一页，留给新的好奇心。', 'The next page is for a new curiosity.')}</p><span aria-hidden="true">✳</span></div>`;
}

function figure(src, alt, caption, type = '') {
  const size = src.includes('/wis-tech-tank/') ? (src.endsWith('.webp') ? [1536,1024] : [1440,1292]) : src.includes('/festival-toolkit/') ? [2139,1356] : src.includes('/axiom/') ? (src.endsWith('.svg') ? [400,280] : [2283,1437]) : src.includes('/folio/') ? [1426,924] : src.includes('hero-asterisk') ? [1254,1254] : [1440,900];
