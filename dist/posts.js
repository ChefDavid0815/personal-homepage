...
function overview() {
  ...
  document.querySelector('meta[name="description"]').content = copy('ChefZC 的随笔：生活、ProjectLens、NBA After Hours、Folio、AXIOM、Festival Toolkit、WIS TECH TANK 与篮球建模的制作手记。', 'Notes by ChefZC: life, ProjectLens, NBA After Hours, Folio, AXIOM, Festival Toolkit, WIS TECH TANK and the Courtside 3D collection.');
...
function figure(src, alt, caption, type = '') {
  const size = src.includes('/projectlens/') ? (src.endsWith('.svg') ? [1600,900] : [1264,860]) : src.includes('/models/') ? [1600,1100] : src.includes('/wis-tech-tank/') ? (src.endsWith('.webp') ? [1536,1024] : [1440,1292]) : src.includes('/festival-toolkit/') ? [2139,1356] : src.includes('/axiom/') ? (src.endsWith('.svg') ? [400,280] : [2283,1437]) : src.includes('/folio/') ? [1426,924] : src.includes('hero-asterisk') ? [1254,1254] : [1440,900];
