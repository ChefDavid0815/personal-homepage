...
function reader() {
  ...
  root.innerHTML = `<nav class="reader-topline" aria-label="${copy('文章导航', 'Article navigation')}"><a href="./posts.html">← ${copy('所有随笔', 'ALL POSTS')}</a><span>NOTE ${post.number} / 03</span><span>${readTime(post)}</span></nav>
    <article class="article-card article-card--${post.theme}" aria-labelledby="article-title">
      <header class="article-header"><div class="article-meta"><span>${esc(text.category)}</span><time datetime="${post.date}">2026.09.17</time></div><h1 id="article-title">${displayTitle}</h1><div class="article-byline"><span class="article-avatar" aria-hidden="true">c_</span><span>ChefZC <small>${copy('写在热爱里', 'NOTES FROM MY WORLD')}</small></span><span class="article-edition">${post.theme === 'signal' ? 'VOL. 01' : 'VERSION 1.0'}</span></div></header>
      <div class="article-art">${artwork(post)}</div>
      <div class="article-reading"><aside class="article-index"><span>${copy('本篇目录', 'ON THIS PAGE')}</span><ol>${text.sections.map((section, i) => `<li><a href="#chapter-${i+1}"><b>0${i+1}</b>${esc(section.title)}</a></li>`).join('')}</ol><span class="article-index-note">${post.theme === 'paper' ? 'COLLECT WITH CARE.' : 'STAY CURIOUS.'}</span></aside>
        <div class="article-body"><p class="article-lead">${esc(text.lead)}</p>
        ${text.sections.map((section, i) => `<section class="article-chapter" id="chapter-${i+1}"><div class="chapter-label"><span>0${i+1}</span><span></span></div><h2>${esc(section.title)}</h2>${section.paragraphs.map(p => `<p>${esc(p)}</p>`).join('')}${i === 0 ? `<blockquote>${esc(text.quote).replace(/\n/g,'<br>')}</blockquote>${figure(post.image, text.imageAlt, text.caption, post.theme === 'signal' ? 'article-figure--sculpture' : '')}` : ''}${section.image ? figure(section.image, section.imageAlt, section.caption) : ''}</section>`).join('')}
