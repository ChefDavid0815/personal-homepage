...
function overview() {
  ...
  document.querySelector('meta[name="description"]').content = copy('ChefZC 的随笔：生活、NBA After Hours、Folio、AXIOM 与 Festival Toolkit 的制作手记。', 'Notes by ChefZC: life, NBA After Hours, Folio, AXIOM and Festival Toolkit.');
...
function figure(src, alt, caption, type = '') {
  const size = src.includes('/festival-toolkit/') ? [2139,1356] : src.includes('/axiom/') ? (src.endsWith('.svg') ? [400,280] : [2283,1437]) : src.includes('/folio/') ? [1426,924] : src.includes('hero-asterisk') ? [1254,1254] : [1440,900];
